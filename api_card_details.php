...
$jwt = new JWT();
$controller = new CardController($conn);

// Get the request method
$request_method = $_SERVER['REQUEST_METHOD'];
$request_uri = parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH);
$segments = explode('/', $request_uri);
$card_id = isset($segments[count($segments) - 1]) && is_numeric($segments[count($segments) - 1]) ? $segments[count($segments) - 1] : null;
...
$card_id_query = isset($_GET['id']) ? $_GET['id'] : null;

// If card_id not found in path, check query parameter
if (!$card_id && $card_id_query) {
    $card_id = $card_id_query;
}
...
switch ($request_method) {
    ...
    case 'PUT':
        ...
        if ($card_id) {
            ...
            echo json_encode(['status' => false, 'message' => 'Card ID is required for update']);
        }
        break;
    ...
    case 'DELETE':
        ...
        if ($card_id) {
            ...
            http_response_code(400);
            echo json_encode(['status' => false, 'message' => 'Card ID is required for delete']);
        }
        break;
